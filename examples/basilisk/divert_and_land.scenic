"""MODE C (full lander-like): miss trajectory and/or attitude → divert → land.

Scenic samples starts that may be headed *away* from the body and/or miss-pointed.
``LanderGuidanceAction('divert')`` velocity-targets toward the site, slews the
boresight at a limited rate (RW stand-in), then soft-brakes in a terminal corridor.

One thruster only — divert and brake share the same force axis (realistic for
this demo hardware).
"""
...
asteroid = new ProceduralAsteroid at (Range(-25, 25), Range(-25, 25), -150 + Range(-15, 15)),
    with radiusX Range(38, 62),
    with radiusY Range(32, 55),
    with radiusZ Range(28, 50),
    with subdivisions 3,
    with detailSeed Range(0, 1e9),
    with noiseAmp Range(1.2, 3.2),
    with terrain (
        [new AsteroidBump for _ in range(18)]
        + [new AsteroidCrater for _ in range(14)]
...
# Position near the body, but velocity can be lateral / outbound (the hard case).
ego = new Spacecraft at (Range(-40, 40), Range(-40, 40), Range(40, 110)),
...
require distance from ego to asteroid < 280
